        ...
        public override void GameConditionTick()
        {
            ...
            if (ticks - LastCaveInTick > 250 && Rand.Value > .9f) //10% chance per tick as long as it's been 250 ticks or more since the last time..
            {
                Find.Storyteller.incidentQueue.Add(
                    new QueuedIncident(
                        new FiringIncident(BC_DefOf.CaveIn, null, StorytellerUtility.DefaultParmsNow(IncidentCategoryDefOf.Misc, Find.CurrentMap)), ticks, Duration)); //Cave-In!
            ...
            if (Rand.Value > .9) //10% chance per tick for another sound.
                BC_DefOf.EarthquakeSound.PlayOneShot(SoundInfo.OnCamera(MaintenanceType.PerTick));
        }
    }
}
